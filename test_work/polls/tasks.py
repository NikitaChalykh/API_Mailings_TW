...
def send_api_message(contact, message):
    ...
    try:
        response = requests.post(
            'https://probe.fbrq.cloud/v1/send/',
            headers=headers,
            json=json
        )
    ...
    try:
        ...
        for mailing in mailings:
            ...
            if (
                ...
                code = mailing['code']
                text = mailing['text']
                contacts = Contact.objects.filter(tag=tag).filter(code=code)
                for contact in contacts:
                    ...
                    if (
                        current_datetime <= datetime.strptime(
                            mailing['end_send_time'], TIME_FORMAT
                        ) and send_api_message(contact, text)
                    ):
